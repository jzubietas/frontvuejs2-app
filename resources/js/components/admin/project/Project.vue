    ...
    <div class="componentwarpper">
        <div class="warpper">
            ...
            <div class="px-5 mt-5">
                <form class="row">
                    <div class="col-6 p-0">
                        <h6 class="col-6 d-inline-block text-right">
                            Buscar por Nombre de Proyecto:
                        </h6>
                        <input v-model="filter.name" type="text" name="name" id='name' class="form-input col-6 d-inline-block" placeholder='Ingrese busqueda'>

                        <h6 class="col-6 d-inline-block mt-4 text-right">
                            Promotor:
                        </h6>
                        <select v-model="filter.brand" name="brand" id='brand' class="form-input col-6 d-inline-block">
                    ...
                    <div class="col-6 p-0">
                        ...
                        <select v-model="filter.sector" name="sector" id='sector' class="form-input col-8 d-inline-block">
                            <option value="All"> Todos </option>
                            <option v-for="item in data.sectors" :value="item.sector">{{ item.sector }}</option>
                        </select>
                    </div>
                </form>
                ...
                <div class="mt-5">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in projects" :key="item.id" class="cursor-pointer">
        ...
        <confirm-modal v-if="toggleConfirmModal" @confirmed="deleteProject" @closeConfirmModal="closeConfirmModal" :title="confirmModalTitle"></confirm-modal>
        <project-details v-if="toggleProjectDetails" :project="selectedProject" :seller_ratings="selected_seller_ratings" @closeProjectDetails="closeDetails"></project-details>
    </div>
...
export default {
    ...
    methods:{
        ...
        openConfirmDeleteModal(project){
            ...
            this.confirmModalTitle = "Are you sure you want to delete " + project.name + "?";
        ...
        filterProject(){
            ...
            if( this.filter.brand == 'All' && this.filter.category == 'All' ){
                for( var project in this.data.projects){
                    if (this.data.projects[project].name.toLowerCase().includes(this.filter.name.toLowerCase())){
                ...
                for( var project in this.data.projects){
                    if (this.data.projects[project].name.toLowerCase().includes(this.filter.name.toLowerCase()) && this.data.projects[project].brand.toLowerCase() == this.filter.brand.toLowerCase()){
                        filteredproj.push(this.data.projects[project]);
                    }
                }
                this.projects = filteredproj;
                ...
                for( var project in this.data.projects){
                    if (this.data.projects[project].name.toLowerCase().includes(this.filter.name.toLowerCase()) && this.data.projects[project].subcategory2.toLowerCase() == this.filter.category.toLowerCase()){
                        filteredproj.push(this.data.projects[project]);
                    }
                }
                this.projects = filteredproj;
                ...
                for( var project in this.data.projects){
                    if (this.data.projects[project].name.toLowerCase().includes(this.filter.name.toLowerCase()) && this.data.projects[project].brand.toLowerCase() == this.filter.brand.toLowerCase() && this.data.projects[project].subcategory2.toLowerCase() == this.filter.category.toLowerCase()){
                        filteredproj.push(this.data.projects[project]);
                    }
                }
